...
Reach for this skill when someone is operating a **live** Solana protocol. Each request maps to a clear outcome:


| When the operator says…                                    | This skill…                                                        |
| ---------------------------------------------------------- | ------------------------------------------------------------------ |
| *"Set up monitoring / what should I watch?"*               | Derives invariants and stands up alerts **before** anything breaks |
| *"Something looks wrong / are we being drained?"*          | Detects and **classifies** a live incident                         |
| *"We're being exploited — what do I do?"*                  | Runs the incident: **contain**, then **eradicate** the root cause  |
| *"Where did the stolen funds go / can any be frozen?"*     | Runs **fund-tracing forensics** with per-hop freeze scoring        |
| *"Write the post-mortem / how do we stop this recurring?"* | Fills the post-mortem and **feeds the lesson back** to build-time  |


> **Defensive / authorized-operator only.** Trace-and-report and protect-your-own-protocol — never "hack back."
